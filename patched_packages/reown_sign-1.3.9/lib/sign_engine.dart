...
/// SYRIUS PATCH: signature for session request handlers that also accept the
/// JSON-RPC request id, so wallets can correlate approval flows with the
/// pending request instead of fingerprinting params.
typedef SessionRequestHandlerWithId = dynamic Function(
  String topic,
  dynamic params, {
  int? requestId,
});

/// SYRIUS PATCH: invokes a registered session request handler, passing the
/// JSON-RPC request [id] when the handler can accept it. Handlers with the
/// legacy `(topic, params)` shape keep working unchanged.
Future<dynamic> invokeSessionRequestHandler(
  dynamic Function(String, dynamic) handler,
  String topic,
  dynamic params,
  int id,
) async {
  if (handler is SessionRequestHandlerWithId) {
    return await handler(topic, params, requestId: id);
  }
...
class ReownSign implements IReownSign {
  ...
  Future<void> _onSessionRequest(
    ...
    try {
      ...
      if (handler != null) {
        try {
          // SYRIUS PATCH: pass the request id to id-aware handlers and send
          // the handler result back to the dApp directly. Upstream discards
          // the result, which leaves the pending request unanswered.
          ...
          try {
            ...
            // The response may already have been sent through another path
            // (e.g. respondSessionRequest); never follow a success result
            // with an error response.
            core.logger.e(
              '[$runtimeType] failed to respond to session request '
              '${payload.id}: $e',
            );
          }
